...
func (r *FetchResponse) CollectClientMetrics(srcHost string) {
	for topic, partitions := range r.Blocks {
		for partitionID, fetchResponseBlock := range partitions {
			for _, recordSet := range fetchResponseBlock.RecordsSet {
				diff := time.Since(recordSet.RecordBatch.MaxTimestamp)
				metrics.ConsumerLag.WithLabelValues(srcHost, topic, strconv.Itoa(int(partitionID))).Add(float64(diff))
			}
		}
	}
}
...
func (r *FetchResponse) requiredVersion() Version {
	switch r.Version {
	case 0:
		return MinVersion
	case 1:
